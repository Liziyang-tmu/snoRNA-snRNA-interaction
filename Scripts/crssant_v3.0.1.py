...
### can also be used to cluster homo.sam
...
def getalign(readsdict, genesdict):
    ...
    for QNAME in readsdict:
        ...
        for line in lines: #now reads from gap1.sam, normal alignment with 1 gap
            ...
            RNAME,POS,CIGAR=align[2],int(align[3]),align[5]
            STRAND='-'if'{0:012b}'.format(int(align[1]))[-5]=='1' else '+'
            #process homo dimer alignments
            if '2I1D2I1D' in CIGAR: #at least 2 overlap between 2 homo dimer
                seglens = [int(i[:-1]) for i in re.findall('[0-9]+M', CIGAR)] #seg lengths
                overlap = len(re.findall('2I1D', CIGAR))
                info1 = [RNAME,STRAND, POS, POS+seglens[0]+overlap-1,line,[]]
                info2 = [RNAME,STRAND, POS+seglens[0], POS+seglens[0]+seglens[1]+overlap-1,line,[]]
                for i in [floor(POS/10)*10,ceil((POS+seglens[0]+overlap-1)/10)*10]:
                    if (RNAME,STRAND,i) in genesdict:
                        info1[-1].extend(genesdict[(RNAME,STRAND,i)]) #add genename
                for i in [floor((POS+seglens[0])/10)*10,ceil((POS+seglens[0]+seglens[1]+overlap-1)/10)*10]:
                    if (RNAME,STRAND,i) in genesdict:
                        info2[-1].extend(genesdict[(RNAME,STRAND,i)]) #add genename
                alignid = line.split()[0] + '_' + str(linecount)
                aligndict[alignid] = info1+info2
            elif '2I1D' not in CIGAR: # gap1 reads
...
def getcov(bedgraphplus,bedgraphminus):
    ...
    for line in f: #plus strand
        ...
        for i in [10*i for i in range10nt]:
            ...
            covdict[(chrom,'+',i)] = max(covdict[(chrom,'+',i)],float(value))
...
def filter_dgs(dg_align_dict, genealign):
    ...
    for (dg, alignids) in dg_align_dict.items():
        ...
        if num_align>0 and len(set(infolist))>0: filtered_dict[dg] = alignids ## set the cutoff reads number for each DG
...
def create_stats(dg_align_dict, genealign, covdict):
    ...
    dg_stats_dict = {}; ng_dict = {}; overlap1_list1=[]; overlap1_list2=[]
    for (dg, alignids) in dg_align_dict.items():
        ...
        overlap1_list1 = [covdict[(chrom1,strand1,i)] for i in range1 if (chrom1,strand1,i) in covdict]
        overlap1_list2 = [covdict[(chrom2,strand2,i)] for i in range2 if (chrom2,strand2,i) in covdict]
        if overlap1_list1:  overlap1=max(overlap1_list1)
        else: overlap1 = 1
        if overlap1_list2:  overlap2=max(overlap1_list2)
        else: overlap2 = 1
